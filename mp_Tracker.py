# 文件作用：稀疏地图跟踪与频域点云生成；隶属于 FGS-SLAM 的稀疏地图跟踪与频域点云生成模块。
import os
import torch
...
class Tracker(SLAMParameters):
    ...
    def __init__(self, slam):
        ...
        # 与 mapping 进程共享的结果子目录（在入口进程统一创建）
    ...
    def tracking(self):
        ...
        self.rgb_images, self.depth_images = self.get_images(f"{self.dataset_path}/image")
        self.num_images = len(self.rgb_images)
        # ---> 新增这里：在子进程正式启动后，再实例化 C++ 对象 <---
        ...
        # 计算并保存 FPS 与 ATE 到 JSON 文件（写入入口处统一创建的时间戳子文件夹）
        import json
        elapsed = time.time() - self.total_start_time
        fps = 1 / (elapsed / self.num_images) if self.num_images > 0 else 0.0
        ate = self.evaluate_ate(self.trajmanager.gt_poses, self.poses) * 100.0

        result_dir = self.output_dir
        os.makedirs(result_dir, exist_ok=True)
        # 从文件夹名 <时间戳>_<数据集名> 解析回字段，供 fps_ate.json 内容记录使用
        timestamp, _, dataset_name = os.path.basename(result_dir).partition("_")

        fname = f"fps_ate.json"
        out_path = os.path.join(result_dir, fname)
        data = {"timestamp": timestamp, "dataset": dataset_name, "fps": float(fps), "ate": float(ate)}
        with open(out_path, "w") as f:
            json.dump(data, f, indent=2)

        # 保存估计轨迹文件（TUM格式）
        est_traj_path = os.path.join(result_dir, "estimated_trajectory.txt")
        with open(est_traj_path, "w") as f:
            f.write("# estimated trajectory\n")
            f.write("# timestamp tx ty tz qx qy qz qw\n")
            for i, pose in enumerate(self.poses):
                tx, ty, tz = pose[:3, 3]
                q = Rotation.from_matrix(pose[:3, :3]).as_quat()  # xyzw -> qx qy qz qw
                f.write(f"{i} {tx} {ty} {tz} {q[0]} {q[1]} {q[2]} {q[3]}\n")
        print(f"Estimated trajectory saved to {est_traj_path}")

        print(f"System FPS: {fps:.2f}")
        print(f"ATE RMSE: {ate:.2f}")
    ...
    def get_images(self, images_folder):
        ...
        if self.trajmanager.which_dataset == "replica":
            ...
            for key in tqdm(image_files):
                ...
                rgb_image = cv2.imread(f"{self.dataset_path}/image/{image_name}.jpg")
                depth_image = np.array(o3d.io.read_image(f"{self.dataset_path}/depth/{depth_image_name}.png"))
    ...
    # 功能：执行 quaternion_multiply 对应的计算或状态操作。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    #   - q1：q1 所表示的计算输入，具体类型、形状和约束见调用上下文。
    #   - Q2：Q2 所表示的计算输入，具体类型、形状和约束见调用上下文。
    # 输出：返回计算、读取或组装得到的结果。
    ...
    # 功能：执行 downsample_and_get_tracking_and_mapping_pointcloud 对应的计算或状态操作。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    ...
    # 功能：执行 initial_opacities_scales 对应的计算或状态操作。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    #   - z_values：z_values 所表示的计算输入，具体类型、形状和约束见调用上下文。
    #   - opacity_mask：高斯不透明度。
    #   - colors：颜色值或颜色张量。
    #   - downsample_idxs：downsample_idxs 所表示的计算输入，具体类型、形状和约束见调用上下文。
    ...
    # 功能：执行 eliminate_overlapped2 对应的计算或状态操作。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    #   - distances：distances 所表示的计算输入，具体类型、形状和约束见调用上下文。
    #   - threshold：判定阈值。
    # 输出：返回计算、读取或组装得到的结果。
    ...
    # 功能：执行 align 对应的计算或状态操作。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    #   - model：model 所表示的计算输入，具体类型、形状和约束见调用上下文。
    #   - data：data 所表示的计算输入，具体类型、形状和约束见调用上下文。
    # 输出：返回计算、读取或组装得到的结果。
    ...
    # 功能：对齐估计与真值轨迹并计算平均平移误差。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    #   - gt_traj：gt_traj 所表示的计算输入，具体类型、形状和约束见调用上下文。
    #   - est_traj：est_traj 所表示的计算输入，具体类型、形状和约束见调用上下文。
    # 输出：返回计算、读取或组装得到的结果。
    ...
    # 功能：执行 gaussian_lp 对应的计算或状态操作。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    #   - Dis：Dis 所表示的计算输入，具体类型、形状和约束见调用上下文。
    #   - D0：D0 所表示的计算输入，具体类型、形状和约束见调用上下文。
    # 输出：返回计算、读取或组装得到的结果。
    def gaussian_lp(self, Dis, D0):
        return torch.exp(-Dis ** 2 / (2 * D0 ** 2))

    # gaussian high-pass filter
    # 功能：执行 gaussian_hp 对应的计算或状态操作。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    #   - Dis：Dis 所表示的计算输入，具体类型、形状和约束见调用上下文。
    #   - D0：D0 所表示的计算输入，具体类型、形状和约束见调用上下文。
    ...
    # 功能：执行 gaussian_bp 对应的计算或状态操作。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    #   - Dis：Dis 所表示的计算输入，具体类型、形状和约束见调用上下文。
    #   - W：W 所表示的计算输入，具体类型、形状和约束见调用上下文。
    #   - C：C 所表示的计算输入，具体类型、形状和约束见调用上下文。
    # 输出：返回计算、读取或组装得到的结果。
    ...
    # 功能：准备与 prepare_filter_data 对应的数据或状态。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    # 输出：返回计算、读取或组装得到的结果。
    ...
    # 功能：从 RGB 图像计算傅里叶高频响应和频域掩码。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    #   - current_image：current_image 所表示的计算输入，具体类型、形状和约束见调用上下文。
    # 输出：返回计算、读取或组装得到的结果。
    ...
    # 功能：对频谱高通滤波、逆变换并进行三角阈值分割。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    #   - freq：freq 所表示的计算输入，具体类型、形状和约束见调用上下文。
    #   - distance：distance 所表示的计算输入，具体类型、形状和约束见调用上下文。
    #   - step_list：step_list 所表示的计算输入，具体类型、形状和约束见调用上下文。
    #   - ii：ii 所表示的计算输入，具体类型、形状和约束见调用上下文。
    # 输出：返回计算、读取或组装得到的结果。
    ...
    # 功能：执行 matrix_to_quaternion 对应的计算或状态操作。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    #   - matrix：matrix 所表示的计算输入，具体类型、形状和约束见调用上下文。
    # 输出：返回计算、读取或组装得到的结果。
    ...
    # 功能：执行 _sqrt_positive_part 对应的计算或状态操作。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    #   - x：x 所表示的计算输入，具体类型、形状和约束见调用上下文。
    # 输出：返回计算、读取或组装得到的结果。
