...
public:
   ...
   */
// 函数作用：执行 TemplatedDatabase 对应的构造、计算或状态操作。
// 所属模块：BoW/BoWG 检索。
// 输入：
//   - use_di：use_di 所表示的计算输入；具体类型和约束由函数签名及调用上下文确定。
//   - di_levels：词典树或图像金字塔层级。
...
// 所属模块：BoW/BoWG 检索。
// 输入：
//   - db：db 所表示的计算输入；具体类型和约束由函数签名及调用上下文确定。
// 输出：无返回值；结果通过对象状态、输出参数、文件或控制台产生。
...
// 输出：无返回值；结果通过对象状态、输出参数、文件或控制台产生。
   ...
   */
  template<class T>
// 函数作用：设置 setVocabulary 对应的数据或状态。
// 所属模块：BoW/BoWG 检索。
// 输入：
//   - voc：视觉词典对象。
...
// 输出：返回函数声明类型规定的计算结果；若为 void，则通过对象状态或输出参数产生结果。
   ...
   * @return vocabulary
   */
// 函数作用：获取 getVocabulary 对应的数据或状态。
// 所属模块：BoW/BoWG 检索。
// 输入：无。
// 输出：返回函数声明类型规定的计算结果；若为 void，则通过对象状态或输出参数产生结果。
   ...
   */
// 函数作用：执行 allocate 对应的构造、计算或状态操作。
// 所属模块：BoW/BoWG 检索。
// 输入：
//   - nd：nd 所表示的计算输入；具体类型和约束由函数签名及调用上下文确定。
//   - ni：ni 所表示的计算输入；具体类型和约束由函数签名及调用上下文确定。
// 输出：返回函数声明类型规定的计算结果；若为 void，则通过对象状态或输出参数产生结果。
...
//   - fvec：fvec 所表示的计算输入；具体类型和约束由函数签名及调用上下文确定。
// 输出：返回函数声明类型规定的计算结果；若为 void，则通过对象状态或输出参数产生结果。
...
//   - fec：fec 所表示的计算输入；具体类型和约束由函数签名及调用上下文确定。
// 输出：返回函数声明类型规定的计算结果；若为 void，则通过对象状态或输出参数产生结果。
  EntryId add(const BowVector &vec, 
    const FeatureVector &fec = FeatureVector() );

// 函数作用：执行 delete_entry 对应的构造、计算或状态操作。
// 所属模块：BoW/BoWG 检索。
// 输入：
//   - entry_id：节点、词或条目 ID。
// 输出：返回函数声明类型规定的计算结果；若为 void，则通过对象状态或输出参数产生结果。
...
// 输出：返回函数声明类型规定的计算结果；若为 void，则通过对象状态或输出参数产生结果。
   ...
   */
// 函数作用：执行 usingDirectIndex 对应的构造、计算或状态操作。
// 所属模块：BoW/BoWG 检索。
// 输入：无。
// 输出：返回函数声明类型规定的计算结果；若为 void，则通过对象状态或输出参数产生结果。
   ...
   * @return di levels
   */
// 函数作用：获取 getDirectIndexLevels 对应的数据或状态。
// 所属模块：BoW/BoWG 检索。
// 输入：无。
// 输出：返回函数声明类型规定的计算结果；若为 void，则通过对象状态或输出参数产生结果。
...
// 输出：返回函数声明类型规定的计算结果；若为 void，则通过对象状态或输出参数产生结果。
   ...
   *   the given entry
   */
// 函数作用：执行 retrieveFeatures 对应的构造、计算或状态操作。
// 所属模块：BoW/BoWG 检索。
// 输入：
//   - id：条目、词或节点标识符。
// 输出：返回函数声明类型规定的计算结果；若为 void，则通过对象状态或输出参数产生结果。
...
// 函数作用：查询 queryChiSquare 对应的数据或状态。
// 所属模块：BoW/BoWG 检索。
// 输入：
//   - vec：vec 所表示的计算输入；具体类型和约束由函数签名及调用上下文确定。
//   - ret：查询结果输出容器。
...
// 函数作用：查询 queryBhattacharyya 对应的数据或状态。
// 所属模块：BoW/BoWG 检索。
// 输入：
//   - vec：vec 所表示的计算输入；具体类型和约束由函数签名及调用上下文确定。
//   - ret：查询结果输出容器。
...
// 函数作用：查询 queryDotProduct 对应的数据或状态。
// 所属模块：BoW/BoWG 检索。
// 输入：
//   - vec：vec 所表示的计算输入；具体类型和约束由函数签名及调用上下文确定。
//   - ret：查询结果输出容器。
