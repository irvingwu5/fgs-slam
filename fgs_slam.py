# 文件作用：系统入口与双进程调度；隶属于 FGS-SLAM 的系统入口与双进程调度模块。
"""
...
class FGS_SLAM(SLAMParameters):
    ...
    def __init__(self, args):
        ...
        test_rgb_img, test_depth_img = self.get_test_image(f"{self.dataset_path}/image")
    ...
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
    #   - rank：rank 所表示的计算输入，具体类型、形状和约束见调用上下文。
    # 输出：无显式返回值；输出体现为状态或外部资源更新。
    ...
    def get_test_image(self, images_folder):
        ...
        if self.camera_parameters[8] == "replica":
            images_folder = os.path.join(self.dataset_path, "image")
            ...
            depth_image = np.array(o3d.io.read_image(f"{self.dataset_path}/depth/{depth_image_name}.png")).astype(np.float32)
    ...
    # 功能：执行 downsample_and_make_pointcloud 对应的计算或状态操作。
    # 输入：
    #   - self：当前类实例，提供并更新对象状态。
